...
SELECT nomEtudiant, prenomEtudiant, dateNaissanceEtudiant FROM etudiants WHERE dateNaissanceEtudiant <( SELECT dateNaissanceEtudiant FROM etudiants WHERE prenomEtudiant = "Jules" AND nomEtudiant = "LECLERCQ" );

--B)Les noms et notes des étudiants qui ont,à l''épreuve 4, une note supérieure à la moyenne de cette épreuve
SELECT idEpreuve, avoir_note.idEtudiant,nomEtudiant,note FROM Avoir_note INNER JOIN Etudiants ON Avoir_note.idEtudiant = Etudiants.idEtudiant 
...
SELECT nomEtudiant FROM etudiants INNER JOIN avoir_note  ON etudiants.idEtudiant = avoir_note.idEtudiant WHERE note >=12;

----D)Le nom des étudiants qui ont dans l''épreuve 4 une note supérieure à celle obtenue par « LUC DUPONT »(à cette même épreuve).
SELECT etudiants.nomEtudiant, etudiants.prenomEtudiant, note FROM avoir_note INNER JOIN etudiants  ON avoir_note.idEtudiant = etudiants.idEtudiant 
